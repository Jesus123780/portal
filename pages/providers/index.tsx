...
import FilterSection from '@/modules/Providers/FilterSection';
import TableSection from '@/modules/Providers/TableSection';
import { PROVIDERS_MENU_CODE } from '@/utils/permissions';
import { useSelector } from '@/store';
import { useEffect, useState } from 'react';
import Status403 from '@/components/403';
import ChartSection from '@/modules/Providers/ChartSection';
...
function ConsultDashboard() {
  ...
  useEffect(() => {
    const isAllow = permissions.find((p) => {
      return p.CodigoPrivilegio === PROVIDERS_MENU_CODE;
    });
  ...
  return (
    <>
      ...
      <AccentHeaderLayout>
        <PageTitleWrapper>
          <PageHeader subtitle="Dashboard de Proveedor" />
        </PageTitleWrapper>
        <Grid
          sx={{
            px: 4
