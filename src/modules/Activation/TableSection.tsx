...
const TableSection = ({ objectFilter }) => {
  ...
  const TableData = useCallback(
    async (filters, page, rowsPerPage, search = '') => {
      // TODO: no api
      const res = await bacFetch('/api/connection/GetLoadDataByFilterActivation', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          params: `'${filters.startDate}','${filters.endDate}','${filters.providerSelected}','${filters.LocalizeSelected}','${filters.SLASelected}',null,'0','${search}','${page}','${rowsPerPage}','0','asc','${filters.listStatusSelected}' `
        })
      });
