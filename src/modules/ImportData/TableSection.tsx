...
const TableSection = ({ objectFilter, refreshData, setRefreshData }) => {
  ...
  const TableData = useCallback(
    async (StartDate, EndDate, Month, Find, Page, RowsPerPage) => {
      const res = await bacFetch('/api/DataImport/GetInfoLoadData', {
        method: 'POST',
        headers: {
  ...
  const LoadDataProcess = useCallback(async (id) => {
    // TODO: no api
    const res = await bacFetch('/api/DataImport/DataLoadProcess', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({
        id: id,
        msg: 'Estimado Cliente se ha asignado proveedor',
        url: 'localhost'
      })
    });
    const jsonData = await res.json();
  ...
  const InfoLoadData = useCallback(async (id) => {
    // TODO: no api
    const res = await bacFetch('/api/connection/GetLoadDataByLoadIdNew', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
  ...
  const DeleteLoadData = useCallback(async (id) => {
    const res = await bacFetch('/api/DataImport/DeleteAllLoadData', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
