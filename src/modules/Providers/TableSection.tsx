...
const TableSection = ({ objectFilter }) => {
  ...
  const TableData = useCallback(
    async (filters, page, rowsPerPage, search = '') => {
      // TODO: no api
      const res = await bacFetch('/api/connection/GetLoadDataByFilterProviderNew', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          params:`'${filters.startDate}','${filters.endDate}','','${filters.LocalizeSelected}','${filters.SLASelected}','','','${page}','${rowsPerPage}','${search}','0','asc','${filters.listStatusSelected}' `
        })
      });
        ...
        >
          <Grid style={{ display: 'flex' }} md={12}>
            ...
            <Grid item md={3} style={{ paddingLeft: 10 }}>
              <FormControl fullWidth disabled={selectedItems.length === 0 || loaderChangeOfficer}>
            ...
            <Grid item md={3} style={{ paddingLeft: 10, display: 'flex' }}>
              <Button
                startIcon={
