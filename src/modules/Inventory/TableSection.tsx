...
const TableSection = ({ objectFilter }) => {
  ...
  const TableData = useCallback(
    async (filters, page, rowsPerPage, search = '') => {
      const res = await bacFetch('/api/connection/GetLoadInventoryByFilter', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          params: `'${filters.startDate}','${filters.endDate}','${filters.custodySelected}',${page},${rowsPerPage},'${search}','0','asc','${filters.listStatusSelected}'`
        })
      });
            ...
            {selectedItems.length > 0 && (
              <>
                <Grid
                  item
                  md={6}
                  style={{ paddingLeft: 10, display: 'flex', gap: 16 }}
                >
                  <FormControl
                    fullWidth
                      ...
                      {data.map((row, index) => {
                        return (
                          ...
                          >
                            ...
                            <TableCell align="center">{row.custody_name}</TableCell>
                            ...
                            <TableCell align="center">
                              <Button
                                ...
                                color="secondary"
                                onClick={() => {
                                  setSelectedDeliveryId(row.inventory_id);
                                  setOpen(true);
                                }}
                              >
                                <RemoveRedEyeOutlined />
