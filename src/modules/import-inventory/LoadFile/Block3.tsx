...
import DetailModal from '@/components/DetailModal/InventoryLoadModalDetail';
...
const Block3 = ({ setRefreshData }) => {
  ...
  const importExcel = useCallback(async (excelData) => {
    const res = await bacFetch('/api/DataImport/ImportInventory', {
      method: 'POST',
      headers: {
    ...
    >
      ...
      >
        ...
        <Typography variant="h5" style={{ textAlign: 'center' }}>
          Permite cargar archivo de inventario
        </Typography>
        <Button
          style={{ marginTop: 20 }}
          color="warning"
